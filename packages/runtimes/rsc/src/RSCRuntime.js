  ...
  apply({bundle, bundleGraph}) {
    ...
    bundle.traverse((node) => {
      if (node.type === 'dependency') {
        ...
        // Server dependency on a Resources component.
        } else if (node.value.env.isServer() && node.value.specifier === '@parcel/runtime-rsc') {
          ...
          for (let b of bundles) {
            ...
            if (b.type === 'css') {
              code += `    <link rel="stylesheet" href=${JSON.stringify(url)} precedence="default" />\n`;
            } else if (b.type === 'js') {
              code += `    <script type="module" src=${JSON.stringify(url)} />\n`;
            }
          }
      ...
      } else if (Array.isArray(node.value.meta.directives) && node.value.meta.directives.includes('use client-entry')) {
